...
function persistDemoAuthIntent(storyId: string, guestId?: string) {
...
export function StoryDemoPreviewClient({ storyId }: { storyId: string }) {
  ...
  useEffect(() => {
    ...
    // Prefer query guest id for this preview — stale storage from another demo breaks migrate.
    const nextGuestId = fromQuery || fromStorage || "";
  ...
  const loadResult = useCallback(async () => {
    ...
    if (window.localStorage.getItem(STORY_AUTH_FROM_RESULT_KEY) === storyId) return;
  ...
  useEffect(() => {
    if (!isAuthenticated || finishingAuthRef.current) return;
    ...
    const guest = guestId || window.localStorage.getItem(STORY_GUEST_ID_KEY) || "";

    void (async () => {
      if (guest) {
        try {
          await fetch("/api/storystudio/migrate", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ guestId: guest })
          });
        } catch {
          // migration is best-effort — cabinet will retry
        }
      }

      window.localStorage.removeItem(STORY_AUTH_FROM_RESULT_KEY);
      window.location.assign(buildStoryCabinetFromDemoUrl(storyId));
    })();
  }, [storyId, isAuthenticated, guestId]);
  ...
  }, [guestId, isAuthenticated, loadResult, storyId]);

  function requireAuth() {
    persistDemoAuthIntent(storyId, guestId);
    router.push(`/register?callbackUrl=${encodeURIComponent(buildStoryCabinetFromDemoUrl(storyId))}`);
  }

  function continueWithAccount() {
    persistDemoAuthIntent(storyId, guestId);
    router.push(`/login?callbackUrl=${encodeURIComponent(buildStoryCabinetFromDemoUrl(storyId))}`);
  }
  ...
  return (
    <div className="min-h-screen bg-[#07050d]">
      ...
      <div className="mx-auto max-w-3xl px-4 pb-20 pt-24 sm:px-6">
        {migrating && isAuthenticated ? (
          <div className="flex h-64 flex-col items-center justify-center gap-3 text-center">
            <Loader2 className="h-8 w-8 animate-spin text-violet" />
            <p className="text-sm text-muted">Переносим проект в кабинет...</p>
          </div>
        ) : loading ? (
          ...
          <div className="space-y-4 rounded-card border border-red-500/30 bg-red-500/10 p-6 text-center">
            <p className="text-red-300">{error}</p>
            <div className="flex flex-col justify-center gap-3 sm:flex-row">
              <Button type="button" onClick={() => void loadResult()} variant="secondary">
                Попробовать снова
              </Button>
              <Link href={buildStoryCabinetFromDemoUrl(storyId)}>
                <Button type="button" className="!border-violet !bg-violet !text-white">
                  Открыть кабинет
                </Button>
              </Link>
            </div>
          ...
          <div className="space-y-4 rounded-card border border-white/10 bg-card/80 p-6 text-center">
            <p className="text-muted">Превью истории недоступно.</p>
            <div className="flex flex-col justify-center gap-3 sm:flex-row">
              ...
              <Button type="button" className="!border-violet !bg-violet !text-white" onClick={requireAuth}>
                Создать аккаунт и продолжить
              </Button>
            </div>
          </div>
