...
export function StoryStudioVideoDemo() {
  ...
  return (
    <div className="mx-auto w-full max-w-[200px] sm:max-w-[220px] lg:max-w-[230px]">
      <div className="overflow-hidden rounded-[24px] border border-violet/30 bg-black shadow-[0_24px_80px_rgba(107,76,255,0.25)]">
        <div className="relative aspect-[9/16] w-full bg-[#0a0812]">
          <video
            ...
            playsInline
            preload="metadata"
            src={STORYSTUDIO_VIDEO_DEMO.src}
          ...
          <div className="pointer-events-none absolute inset-x-0 top-0 bg-gradient-to-b from-black/50 to-transparent p-4">
            <div className="flex items-center justify-between gap-2">
              <span className="rounded-full border border-violet/30 bg-violet/20 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-violet">
                {STORYSTUDIO_VIDEO_DEMO.badge}
              </span>
              <span className="text-[10px] font-medium text-white/70">{STORYSTUDIO_VIDEO_DEMO.durationLabel}</span>
            </div>
          </div>

          <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent p-4">
            <p className="text-sm font-semibold text-white">{STORYSTUDIO_VIDEO_DEMO.title}</p>
      ...
      <p className="mt-3 text-center text-xs text-muted lg:text-left">
        Реальный пример видео-серии из портрета персонажа
      </p>
    </div>
  );
}
