...
export function WildberriesBetaNotice({ compact = false, className = "" }: WildberriesBetaNoticeProps) {
  return (
    <div
      className={`flex items-start gap-3 rounded-[16px] border border-amber-500/25 bg-amber-500/10 ${
        compact ? "px-3 py-2.5" : "px-4 py-3"
      } ${className}`.trim()}
    >
      <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-amber-500/20 px-2 py-0.5 text-[10px] font-black uppercase tracking-[0.14em] text-amber-700">
        Beta
      </span>
      <div className="min-w-0">
        <p className={`font-semibold text-ink ${compact ? "text-xs" : "text-sm"}`}>
      ...
      {!compact ? <AlertTriangle className="hidden shrink-0 text-amber-600 sm:block" size={18} /> : null}
    </div>
  );
}
