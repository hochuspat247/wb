...
- title, shortDescription, fullDescription и названия площадок должны описывать ТОТ ЖЕ товар, что в каноническом названии
- fullDescription: развёрнутое описание товара 500–1500 символов про сам товар (не про дизайн карточки)
- characteristics: минимум 6 пунктов, если во входных данных есть хоть какие-то факты; не оставляй карточку с 2–3 пустыми полями
...
- не пиши универсальные шаблоны вроде «помощник для ежедневных тренировок», если товар к этому не относится

Wildberries:
- wbName: точное торговое название ЭТОГО товара (как на витрине), без площадок и без системных слов
- wbDescription: полноценное описание товара для карточки WB, 600–1800 символов:
  что это за товар, для чего он, из чего сделан (если известно), кому подходит, как использовать по факту товара,
  что важно покупателю перед заказом. Пиши про реальный товар из входных данных, а не про похожий другой SKU
- ЗАПРЕЩЕНО подменять товар: если это воздушный шар / надувная фигура — нельзя писать про тренировки, мячи, спорт, фитнес
- wbCharacteristics: минимум 6–10 характеристик в формате ключ/значение из известных фактов
  (Тип товара, Материал, Назначение, Цвет, Размер/диаметр, Комплектация, Для кого, Особенность формы и т.п.).
  Если факт неизвестен — не выдумывай число/бренд, пропусти поле или напиши нейтрально «уточняйте у продавца» только если это критично
- wbSafeImageTexts: 4–6 коротких фраз именно про свойства товара
- wbPhotoRules / wbForbiddenImageTexts / wbQualityTips: заполнить полностью
- не добавляй в title/wbName: ${wbProfile.forbiddenTitleWords.map((w) => `"${w}"`).join(", ")}
